...
function printConfiguration() {
    ...
    WEBHOOK_FETCH_TIMEOUT:  ${process.env.WEBHOOK_FETCH_TIMEOUT}
    LINUX_DIRECT_FILE:      ${process.env.LINUX_DIRECT_FILE}
    DARWIN_DIRECT_FILE:     ${process.env.DARWIN_DIRECT_FILE}
    DARWIN_ZIP_FILE:        ${process.env.DARWIN_ZIP_FILE}
    WIN32_RELEASES_FILE:    ${process.env.WIN32_RELEASES_FILE}
    WIN32_NUPKG_FILE:       ${process.env.WIN32_NUPKG_FILE}
    WIN32_DIRECT_FILE:      ${process.env.WIN32_DIRECT_FILE}
    ===============================================
`);

    if (!process.env.GITHUB_REPO) {
        console.log('GITHUB_REPO required. Exiting.');
        process.exit(1);
    }

    if (!process.env.GITHUB_TOKEN) {
        console.log('GITHUB_TOKEN required. Exiting.');
        process.exit(1);
    }
}
...
function keepAlive() {
    setInterval(function() {
        ...
        console.log();
        console.log(`    KEEPALIVE:         ${new Date().toJSON()}`);
        console.log(`    VERSIONS:          ${versions.join(', ')}`);
    }, (1000 * 60) * pollingInterval );
}
